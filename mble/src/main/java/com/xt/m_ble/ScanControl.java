...
class ScanControl {
    ...
    private MyBleScanRuleConfig mScanRuleConfig;
    private List<MyLeScanCallback> mMyLeScanCallbacks = new ArrayList<>();
    ...
    ScanControl(BluetoothAdapter bluetoothAdapter) {
        ...
        mScanRuleConfig = new MyBleScanRuleConfig.Builder()
...
//                .setScanTimeOut(20000)              // 扫描超时时间，可选，默认10秒
    ...
    void onStartLeScanCallback(boolean isStartScanSuccess, String desc) {
        for (MyLeScanCallback myLeScanCallback : mMyLeScanCallbacks) {
            myLeScanCallback.onStartLeScanCallback(isStartScanSuccess, desc);
        }
    }
    ...
    private synchronized void stopLeScan(String text) {
        if (mBluetoothAdapter != null) {
            ...
            for (MyLeScanCallback myLeScanCallback : mMyLeScanCallbacks) {
                myLeScanCallback.onStopLeScanCallback();
            }
//            ToastUtils.showShort(text);
    ...
    private BluetoothAdapter.LeScanCallback mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
        ...
        public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
            checkDevice(device);
            for (MyLeScanCallback item : mMyLeScanCallbacks) {
    ...
    void addMyLeScanCallback(MyLeScanCallback myLeScanCallback) {
        mMyLeScanCallbacks.add(myLeScanCallback);
    }

    void removeMyLeScanCallback(MyLeScanCallback myLeScanCallback) {
        mMyLeScanCallbacks.remove(myLeScanCallback);
    }
}
